            ...
            <div class="table-responsive">
                ...
                <table class="table text-nowrap table-hover table-striped">
                    ...
                    <tbody class="p-0">
                        ...
                        <tr>
                            <td class="text-center">{{$streamCourse->id}}</td>
                            <td>{{$streamCourse->course->name}}</td>
                            <td>{{$streamCourse->course->code}}</td>
                            <td>{{$streamCourse->credit_hour}}</td>
                            <td>{{$streamCourse->ects}}</td>
                            <td>{{$streamCourse->course->coordinator}}</td>
                            <td>
                                <form class="col-12 p-0" action="{{ route('student.course.store') }}"
                                    onsubmit="return confirm('Are you sure')" method="POST">
                                    @csrf
                                    <input type="hidden" name="course_id" value="{{ $streamCourse->course_id }}">
                                    <button class="m-btn btn-block m-btn--pill btn btn-sm btn-primary">
                                        Add Course
                                    </button>
                                </form>
                            </td>
